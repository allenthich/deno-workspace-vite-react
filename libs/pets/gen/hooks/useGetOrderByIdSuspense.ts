...
import type { GetOrderByIdQueryResponse, GetOrderByIdPathParams, GetOrderById400, GetOrderById404 } from "../types/GetOrderById.ts";
...
async function getOrderById(orderId: GetOrderByIdPathParams["orderId"], config: Partial<RequestConfig> = {}) {
    const res = await client<GetOrderByIdQueryResponse, GetOrderById400 | GetOrderById404, unknown>({ method: "GET", url: `/store/order/${orderId}`, baseURL: "https://petstore3.swagger.io/api/v3", ...config });
    return res.data;
}
 ...
 * @link /store/order/:orderId
 */
export function useGetOrderByIdSuspense<TData = GetOrderByIdQueryResponse, TQueryData = GetOrderByIdQueryResponse, TQueryKey extends QueryKey = GetOrderByIdSuspenseQueryKey>(orderId: GetOrderByIdPathParams["orderId"], options: {
