...
async function getInventory(config: Partial<RequestConfig> = {}) {
    const res = await client<GetInventoryQueryResponse, Error, unknown>({ method: "GET", url: `/store/inventory`, baseURL: "https://petstore3.swagger.io/api/v3", ...config });
    return res.data;
}
 ...
 * @link /store/inventory
 */
export function useGetInventorySuspense<TData = GetInventoryQueryResponse, TQueryData = GetInventoryQueryResponse, TQueryKey extends QueryKey = GetInventorySuspenseQueryKey>(options: {
